#!/usr/bin/env python3
"""NCU profiling: Triton vector-add, Config H (B=24, S=48K, H=2048, FP32)

NOTE: N=2.36B exceeds int32 range. Offsets are cast to int64 to avoid overflow.
"""
import os
os.makedirs(os.path.expanduser("~/triton-profiling-tmp"), exist_ok=True)
os.environ["TMPDIR"] = os.path.expanduser("~/triton-profiling-tmp")

import torch
import triton
import triton.language as tl

# -- Config H --
BATCH, SEQ, HIDDEN = 24, 48_000, 2_048
DTYPE = torch.float32
N = BATCH * SEQ * HIDDEN

# -- Triton kernel --
@triton.jit
def _triton_vadd_kernel(a_ptr, b_ptr, c_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(axis=0).to(tl.int64)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE).to(tl.int64)
    mask = offsets < n_elements
    a = tl.load(a_ptr + offsets, mask=mask)
    b = tl.load(b_ptr + offsets, mask=mask)
    tl.store(c_ptr + offsets, a + b, mask=mask)

def triton_vadd(a, b):
    c = torch.empty_like(a)
    n = c.numel()
    grid = lambda meta: (triton.cdiv(n, meta["BLOCK_SIZE"]),)
    _triton_vadd_kernel[grid](a, b, c, n, BLOCK_SIZE=1024)
    return c

if __name__ == "__main__":
    print(f"Config H | Triton | N={N:,} | {DTYPE}")
    if N > 2**31 - 1:
        print(f"Warning: N={N:,} exceeds int32 max, using int64 indexing")
    free_mem, total_mem = torch.cuda.mem_get_info()
    required = 3 * N * 4
    print(f"GPU memory: {free_mem/1e9:.1f} GB free / {total_mem/1e9:.1f} GB total")
    print(f"Required:   {required/1e9:.1f} GB")
    if required > free_mem * 0.95:
        print("ERROR: Insufficient GPU memory")
        exit(1)

    a = torch.ones(N, device="cuda", dtype=DTYPE)
    b = torch.full((N,), 2.0, device="cuda", dtype=DTYPE)

    for _ in range(2):
        c = triton_vadd(a, b)
    torch.cuda.synchronize()

    c = triton_vadd(a, b)
    torch.cuda.synchronize()

    assert torch.allclose(c, torch.full_like(c, 3.0), atol=1e-5), "FAIL"
    print("Correctness: PASS")
